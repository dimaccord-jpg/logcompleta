...
def _big_result(n_rows: int, n_tables: int, comparison_id: str = "x") -> dict:
    ...
    for r in range(n_rows):
        tr = {
            t["table_id"]: {
                "status": "calculated",
                "calculated_freight": 10.5,
                "components": {"base": 10.0, "extra": 0.5},
                "evidence": {"region": "SP", "rule": "weight"},
            }
            for t in tables
        }
        rows.append(
    ...
    return {
        ...
        "results_by_table": {t["table_id"]: {"rows": rows} for t in tables},
        "summary": {
            "calculated_cell_count": n_rows * n_tables,
            "error_cell_count": 0,

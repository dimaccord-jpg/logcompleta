...
def test_operational_context_semantica_e_linguagem():
    ...
    assert "concentracao operacional" in policy["allowed_language"]
    assert "comportamento atipico" in policy["allowed_language"]
    assert "variacao relevante" in policy["allowed_language"]
    assert "oportunidade de investigacao" in policy["allowed_language"]
    assert "dados insuficientes" in policy["allowed_language"]
    assert "tendencia operacional" in policy["allowed_language"]
    assert "participacao relevante" in policy["allowed_language"]
    assert "erro de cobranca" in policy["forbidden_language"]
    assert "cobranca incorreta" in policy["forbidden_language"]
    assert "transportadora errada" in policy["forbidden_language"]
    assert "valor incorreto" in policy["forbidden_language"]
    assert "divergencia contratual" in policy["forbidden_language"]
    assert "conclusao financeira acusatoria" in policy["forbidden_language"]

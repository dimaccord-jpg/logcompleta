...
def test_apropria_upload_cleide_converte_rows_ms_em_creditos(app):
    with app.app_context():
        seed_sistema_interno()
        seed_cleiton_cost_config()
        conta, franquia = seed_conta_franquia_cliente()
        ...
        with app.test_request_context("/api/cleide/upload", method="POST"):
            ...
            out = apropriar_billing_upload_cleide(
                idempotency_key="cleide-upload-k1:success",
                rows_processed=100,
                processing_time_ms=1000,
                status="success",
            )
...
def test_idempotencia_upload_cleide_nao_duplica_cobranca(app):
    with app.app_context():
        seed_sistema_interno()
        seed_cleiton_cost_config()
        conta, franquia = seed_conta_franquia_cliente()
        ...
        with app.test_request_context("/api/cleide/upload", method="POST"):
            ...
            first = apropriar_billing_upload_cleide(
                idempotency_key="cleide-upload-k2:success",
                rows_processed=100,
                processing_time_ms=1000,
                status="success",
            )
            second = apropriar_billing_upload_cleide(
                idempotency_key="cleide-upload-k2:success",
                rows_processed=100,
                processing_time_ms=1000,
                status="success",

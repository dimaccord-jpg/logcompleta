"""
Backend do chat analítico pós-BI da Agente Compara.
...
from app.agente_compara_doc_service import (
    ...
    agente_compara_insights_chat_idempotency_key,
)
...
from app.agente_compara_insights_prompt import build_insights_user_prompt
from app.agente_compara_insights_query import (
...
INSIGHTS_CHAT_IDEMPOTENCY_CACHE_SESSION_KEY = "agente_compara_insights_chat_idempotency_cache"
...
def sanitize_agente_compara_sender_signature(text: str) -> str:
    """
    Remove assinatura indevida da Agente Compara/AgenteFrete no fechamento da resposta.
    ...
    if trailing:
        ...
        if identity_lines and all(
            re.fullmatch(
                r"(?i)Agente Compara|Agente\s*Frete|Analista\s+de\s+Frete(?:\s+Experiente)?"
                r"|Auditora\s+Virtual|Auditoria\s+Agente Compara",
                line or "",
            )
...
def soften_insights_gemini_output(text: str) -> str:
    ...
    return sanitize_agente_compara_sender_signature(result)
...
def finalize_insights_answer(text: str, *, soften: bool = False) -> str:
    ...
    return sanitize_agente_compara_sender_signature((text or "").strip())
...
def _get_model_candidates() -> list[str]:
    candidates = [
        ...
        "gemini-2.5-flash",
        (os.getenv("AGENTE_COMPARA_INSIGHTS_CHAT_MODEL_FALLBACK") or "").strip(),
        DEFAULT_MODEL_FALLBACK,
    ]
    seen: set[str] = set()
    out: list[str] = []
...
def _get_max_history(*, max_history: int | None = None) -> int:
    ...
    if not has_app_context():
        return max(0, int(get_agente_compara_config().chat_max_history))
    return max(0, int(get_agente_compara_config().chat_max_history))
...
def get_cached_insights_chat_response(session_obj, request_id: str, *, batch_scope: str = "") -> dict | None:
    ...
    if not isinstance(cache, dict):
        return None
    payload = cache.get(agente_compara_insights_chat_idempotency_key(ref, batch_scope))
...
def cache_insights_chat_response(
    ...
    *,
    ...
    if not isinstance(cache, dict):
        cache = {}
    cache_key = agente_compara_insights_chat_idempotency_key(ref, batch_scope)
    ...
    cache_entry = {
        "answer": safe_answer,
        "flow_type": payload.get("flow_type") or AGENTE_COMPARA_INSIGHTS_CHAT_FLOW_TYPE,
    ...
    *,
    ...
    for model in _get_model_candidates():
        try:
            ...
            last_error = exc
            logger.warning(
                "Agente Compara insights chat provider failure: model=%s exc_type=%s message=%s",
                model,
                exc.__class__.__name__,
                exc,
            )
    if last_error:
        logger.exception("Agente Compara insights chat falhou após fallbacks: %s", last_error)
...
def chat_agente_compara_insights_reply(
    user_message: str,
    history: list,
    *,
    session_obj,
    ...
    if cached is not None:
        return {
            ...
            "flow_type": cached.get("flow_type") or AGENTE_COMPARA_INSIGHTS_CHAT_FLOW_TYPE,
    ...
    if conversation_focus is None:
        ...
        existing = session_obj.get("agente_compara_insights_conversation_focus")
    ...
    if intent in {INTENT_EXPLAIN_CALCULATION, INTENT_LOCATE_DOCUMENT}:
        ...
        if target["kind"] == "duplicate":
            from app.agente_compara_insights_query import format_duplicate_document_options
...
def format_ambiguity_safe(bundle: dict) -> str:
    from app.agente_compara_insights_query import format_ambiguity

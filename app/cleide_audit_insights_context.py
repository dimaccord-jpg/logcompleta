...
ERROR_INSIGHTS_NO_TEMP_TABLE = "cleide_audit_insights_no_temp_table"
ERROR_INSIGHTS_TEMP_TABLE_EXPIRED = "cleide_audit_insights_temp_table_expired"
ERROR_INSIGHTS_BATCH_NOT_FOUND = "cleide_audit_insights_batch_not_found"
ERROR_INSIGHTS_BATCH_NOT_PROCESSED = "cleide_audit_insights_batch_not_processed"
ERROR_INSIGHTS_BATCH_NO_RESULTS = "cleide_audit_insights_batch_no_results"
ERROR_INSIGHTS_CHAT_LOCKED = "cleide_audit_insights_chat_locked"

CLEIDE_AUDIT_INSIGHTS_CHAT_UNLOCK_SESSION_KEY = "cleide_audit_insights_chat_unlock"
CLEIDE_AUDIT_INSIGHTS_CONVERSATION_FOCUS_SESSION_KEY = "cleide_audit_insights_conversation_focus"
...
def clear_insights_chat_unlock(session_obj) -> None:
    session_obj.pop(CLEIDE_AUDIT_INSIGHTS_CHAT_UNLOCK_SESSION_KEY, None)
...
def mark_insights_chat_unlocked(session_obj, bundle: dict) -> dict:
    ...
    session_obj[CLEIDE_AUDIT_INSIGHTS_CHAT_UNLOCK_SESSION_KEY] = payload
    if hasattr(session_obj, "modified"):
        session_obj.modified = True
    return payload
...
def is_insights_chat_unlocked(session_obj, bundle: dict) -> bool:
    raw = session_obj.get(CLEIDE_AUDIT_INSIGHTS_CHAT_UNLOCK_SESSION_KEY)
...
def get_conversation_focus(session_obj, batch_scope: str) -> dict | None:
    ...
    raw = session_obj.get(CLEIDE_AUDIT_INSIGHTS_CONVERSATION_FOCUS_SESSION_KEY)
    if not isinstance(raw, dict):
        return None
...
def clear_conversation_focus(session_obj) -> None:
    session_obj.pop(CLEIDE_AUDIT_INSIGHTS_CONVERSATION_FOCUS_SESSION_KEY, None)
    if hasattr(session_obj, "modified"):
        session_obj.modified = True
...
def set_conversation_focus(
    ...
    *,
    ...
    previous = session_obj.get(CLEIDE_AUDIT_INSIGHTS_CONVERSATION_FOCUS_SESSION_KEY)
    ...
    session_obj[CLEIDE_AUDIT_INSIGHTS_CONVERSATION_FOCUS_SESSION_KEY] = payload
    if hasattr(session_obj, "modified"):
        session_obj.modified = True
    return payload

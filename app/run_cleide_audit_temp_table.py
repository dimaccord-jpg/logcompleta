"""
Extração técnica da tabela temporária da Cleide Auditoria (Fase 1).
...
from app.cleide_audit_doc_service import (
    ...
    cleide_audit_temp_table_extraction_idempotency_key,
...
TEMP_TABLE_EXTRACTION_IDEMPOTENCY_CACHE_KEY = "cleide_audit_temp_table_extraction_cache"

DEFAULT_MODEL_FALLBACK = "gemini-2.5-flash-lite"
DEFAULT_EXTRACTION_TIMEOUT_MS = 60_000
CLEIDE_AUDIT_TEMP_TABLE_TIMEOUT_MS_ENV = "CLEIDE_AUDIT_TEMP_TABLE_TIMEOUT_MS"
...
def get_extraction_timeout_ms() -> int:
    ...
    Prioridade: CLEIDE_AUDIT_TEMP_TABLE_TIMEOUT_MS > DEFAULT_EXTRACTION_TIMEOUT_MS.
    Não herda GEMINI_HTTP_TIMEOUT_MS (reservado a chat/outros fluxos).
    """
    raw = (os.getenv(CLEIDE_AUDIT_TEMP_TABLE_TIMEOUT_MS_ENV) or "").strip()
    if raw:
        try:
...
def _get_model_candidates() -> list[str]:
    candidates = [
        ...
        "gemini-2.5-flash",
        (os.getenv("CLEIDE_AUDIT_TEMP_TABLE_MODEL_FALLBACK") or "").strip(),
...
def run_cleide_audit_temp_table_extraction(
    ...
    *,
    ...
    doc_ctx = build_cleide_audit_document_context_for_chat(sess)
    ...
    if not client:
        logger.warning(
            "Cleide temp_table extraction: nenhuma chave Gemini configurada."
        )
    ...
    for model_index, model in enumerate(model_candidates):
        ...
        try:
            response = cleiton_governed_generate_content(
                ...
                contents=contents,
                agent="cleide",
                flow_type=CLEIDE_AUDIT_TEMP_TABLE_EXTRACTION_FLOW_TYPE,
            ...
            logger.warning(
                "Cleide temp_table extraction provider failure: model=%s timeout_ms=%s exc_type=%s message=%s",
                model,
                effective_timeout_ms,
    ...
    if last_error:
        logger.exception(
            "Cleide temp_table extraction falhou após fallbacks: %s", last_error
        )
    if last_timeout:
...
def trigger_temp_table_extraction_for_session(
    *,
    ...
    """
    sess = session_obj if session_obj is not None else session
    source_doc_ids = get_cleide_audit_doc_ids(sess)
    normalized = _normalize_source_doc_ids(source_doc_ids)
    if not normalized:
        return None
    ...
    try:
        return run_cleide_audit_temp_table_extraction(
            normalized,
            session_obj=sess,
            user_scope=user_scope,
            franquia_scope=franquia_scope,
        ...
        logger.exception(
            "Cleide temp_table: falha inesperada na extração pós-upload; marcando failed."
        )
        try:
            return apply_temp_table_extraction_from_model_payload(
                _failed_extraction_payload(reading_alerts=READING_ALERT_PARSER_NO_JSON),
            ...
            logger.exception(
                "Cleide temp_table: falha ao marcar failed após erro inesperado."
            )
            return None

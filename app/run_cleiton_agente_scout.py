...
def _coletar_rss(url: str, max_itens: int, tipo_sugerido: str, fonte_tipo: str = "rss") -> list[dict]:
    ...
    try:
        ...
        if getattr(feed, "bozo", False) and not getattr(feed, "entries", None):
            logger.warning("Feed RSS inválido ou vazio: %s", url[:80])
            return []
        out = []
        ...
        logger.exception("Erro ao coletar RSS %s: %s", url[:80], e)
        return []
...
def _coletar_url_lista(url: str, max_itens: int, tipo_sugerido: str) -> list[dict]:
    ...
    try:
        ...
        logger.exception("Erro ao coletar API/URL %s: %s", url[:80], e)
        return []
...
def executar_coleta() -> dict[str, Any]:
    ...
    Retorna dict com: inseridas, ignoradas_duplicata, erros, fontes_processadas.
    """
    if not _scout_enabled():
        logger.info("Scout desabilitado (SCOUT_ENABLED=false).")
        resultado = {"inseridas": 0, "ignoradas_duplicata": 0, "erros": 0, "fontes_processadas": 0}
        auditoria_registrar(
            tipo_decisao="scout",

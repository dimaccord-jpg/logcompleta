"""
Montagem de contexto documental autorizado para o chat da Cleide Auditoria.

Carrega registros via store Cleiton usando exclusivamente `cleide_audit_doc_ids`.
...
from app.cleide_audit_prompt import build_cleide_audit_document_guidance
...
def build_cleide_audit_document_context_for_chat(session_obj) -> dict:
    """
    Monta bloco de contexto documental para injecao no prompt da Cleide Auditoria.

    Usa exclusivamente IDs em `cleide_audit_doc_ids` da sessao informada.
    """
    cleiton_cfg = get_cleiton_doc_config()
    audit_cfg = get_cleide_audit_config()
    doc_ids = get_cleide_audit_doc_ids(session_obj)
    if not doc_ids:
    ...
    guidance = build_cleide_audit_document_guidance().strip()
    header = f"{guidance}\n\nContexto documental temporario da Cleide Auditoria:\n\n"
    blocks: list[str] = []
    context_truncated = False

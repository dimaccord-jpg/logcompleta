"""
Prompts internos da Agente Compara documental.
...
def build_agente_compara_system_prompt() -> str:
    """Prompt de sistema da Agente Compara — Compare Tabelas."""
    return """
Voce e Agente Compara — Compare Tabelas.
...
def build_agente_compara_document_guidance() -> str:
...
- Os documentos sao contexto temporario da Agente Compara; use apenas evidencias presentes no material preparado.
...
""".strip()
    ...
    return (
        "Bases de cálculo ativas cadastradas no Admin da Agente Compara:\n"
        f"{json.dumps(compact, ensure_ascii=False, separators=(',', ':'))}"
    )


def build_agente_compara_temp_table_technical_prompt(
    ...
    prompt = """
Voce e um extrator tecnico de custos de frete para a Agente Compara.
...
def build_agente_compara_temp_table_fallback_prompt() -> str:
    """Prompt reduzido para fallback do modelo lite após timeout no principal."""
    return """
Voce e um extrator tecnico de frete para a Agente Compara.

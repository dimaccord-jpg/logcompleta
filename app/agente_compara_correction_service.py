"""Correcoes assistidas da Agente Compara.
...
ERROR_CORRECTION_INVALID_PAYLOAD = "agente_compara_correction_invalid_payload"
ERROR_CORRECTION_NO_TEMP_TABLE = "agente_compara_correction_no_temp_table"
ERROR_CORRECTION_SUGGESTION_NOT_FOUND = "agente_compara_correction_suggestion_not_found"
ERROR_CORRECTION_CONSTRAINT_MISMATCH = "agente_compara_correction_constraint_mismatch"
ERROR_CORRECTION_UNSUPPORTED_TRANSFORMATION = "agente_compara_correction_unsupported_transformation"
ERROR_CORRECTION_PREVIEW_NOT_FOUND = "agente_compara_correction_preview_not_found"
ERROR_CORRECTION_PREVIEW_EXPIRED = "agente_compara_correction_preview_expired"
ERROR_CORRECTION_PREVIEW_NOT_SAFE = "agente_compara_correction_preview_not_safe"
ERROR_CORRECTION_UNDO_NOT_FOUND = "agente_compara_correction_undo_not_found"

TRANSFORMATION_SELECT_PRICING_DIMENSION = "select_pricing_dimension"
PREVIEW_TTL_MINUTES = 10
PREVIEW_SESSION_KEY = "agente_compara_correction_previews"
MAX_SESSION_PREVIEWS = 5
CORRECTION_HISTORY_MAX_ITEMS = 5


class AgenteComparaCorrectionError(Exception):
...
def _require(condition: bool, error_code: str, message: str) -> None:
    if not condition:
        raise AgenteComparaCorrectionError(error_code, message)
...
def _repeatable(record: dict, transformed: dict, normalized_rows: list[dict]) -> bool:
    from app.agente_compara_doc_service import compute_audit_outputs
...
def _load_current_record_for_session(*, user_scope=None, franquia_scope=None, action_label: str = "preview") -> dict:
    ...
    if not active_id:
        raise AgenteComparaCorrectionError(ERROR_CORRECTION_NO_TEMP_TABLE, "Nenhuma tabela temporária ativa nesta sessão.")
    ...
    if record is None:
        ...
        raise AgenteComparaCorrectionError(ERROR_CORRECTION_NO_TEMP_TABLE, "Tabela temporária ativa não encontrada.")
    status = (record.get("status") or "").strip().lower()
    if status == TEMP_TABLE_STATUS_EXPIRED:
        raise AgenteComparaCorrectionError(ERROR_AUDIT_EXPIRED, "A tabela temporária desta sessão expirou.")
    if status in {TEMP_TABLE_STATUS_DISCARDED, TEMP_TABLE_STATUS_PROCESSING}:
        raise AgenteComparaCorrectionError(ERROR_AUDIT_NO_TEMP_TABLE, f"Tabela temporária indisponível para {action_label}.")
    try:
        _assert_temp_table_scope(record, user_scope=user_scope, franquia_scope=franquia_scope)
    except AgenteComparaBatchError as exc:
        raise AgenteComparaCorrectionError(exc.error_code, exc.message) from exc
    return record
...
def _suggestion_for_current_record(record: dict, suggestion_id: str) -> dict:
    ...
    if suggestion is None:
        raise AgenteComparaCorrectionError(ERROR_CORRECTION_SUGGESTION_NOT_FOUND, "Sugestão não encontrada para o artefato atual.")
    return suggestion
...
def _audit_batch_with_outputs(audit_batch: dict, outputs: dict, *, now: str, expires_at) -> dict:
    ...
    if fiscal_snapshot:
        from app.agente_compara_doc_service import _apply_tax_fiscal_snapshot_to_audit_batch
...
def apply_audit_correction_for_session(
    *,
    ...
    from flask import session
    from app.agente_compara_doc_service import compute_audit_outputs, save_temp_table_record, _public_temp_table
